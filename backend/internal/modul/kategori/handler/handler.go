...
import (
	...
	"github.com/kasku/backend/internal/modul/kategori/usecase"
)
...
func (h *Handler) Ubah(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, usecase.ErrKategoriTidakAda) {
...
func (h *Handler) Hapus(c *gin.Context) {
	...
	respons.Sukses(c, gin.H{"pesan": "Kategori berhasil dihapus"})
}
...
func (h *Handler) AmbilSemua(c *gin.Context) {
	jenis := c.Query("jenis")
	hasil, err := h.uc.AmbilSemua(middleware.AmbilPenggunaID(c), jenis)
	if err != nil {
		respons.GagalInternal(c, err.Error())
		return
	}
